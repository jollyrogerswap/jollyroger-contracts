...
// File: contracts\RubyToken.sol

pragma solidity 0.6.12;


// RubyToken with Governance.
contract RubyToken is BEP20('RUBY', 'RUBY') {
...
// MasterChef is the master of RUBY. He can make RUBY he is a fair guy.
//
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    // Update the given pool's RUBY allocation point and deposit fee. Can only be called by the owner.
    ...
    function updatePool(uint256 _pid) public {
        ...
        ruby.mint(address(this), rubyReward);
    ...
    function safeRubyTransfer(address _to, uint256 _amount) internal {
        ...
        if (_amount > rubyBal) {
            transferSuccess = ruby.transfer(_to, rubyBal);
        } else {
            transferSuccess = ruby.transfer(_to, _amount);
        }
        require(transferSuccess, "safeRubyTransfer: transfer failed");
    }
    ...
    function updateEmissionRate(uint256 _rubyPerBlock) public onlyOwner {
        massUpdatePools();
        rubyPerBlock = _rubyPerBlock;
        emit UpdateEmissionRate(msg.sender, _rubyPerBlock);
    }
}
